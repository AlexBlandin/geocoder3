...
class OneResult:
  ...
  def __repr__(self):
    ...
    if self.address:
      return f"[{six.text_type(self.address)}]"
    else:
    ...
    with StringIO() as output:
      ...
      for key in self.osm:
        if "addr:" in key:
          if self.json.get(key.replace("addr:", "")):
            ...
            print(f"- [ ] {key}\n", file=output)
      print(f"({osm_count}/{len(self.osm) - 2})\n", file=output)
      print("\n", file=output)
      print("Fieldnames\n", file=output)
      print("----------\n", file=output)
      fields_count = 0
      for fieldname in self.fieldnames:
        if self.json.get(fieldname):
          ...
          print(f"- [ ] {fieldname}\n", file=output)
      print(f"({fields_count}/{len(self.fieldnames)})\n", file=output)
...
class MultipleResultsQuery(MutableSequence):
  ...
  def __repr__(self):
    base_repr = f"<[{self.status}] {self.provider.title()} - {self.method.title()} {{0}}>"
    if len(self) == 0:
  ...
  def _connect(self):
    ...
    try:
      ...
      self.response = response = self.rate_limited_get(self.url, params=self.params, headers=self.headers, timeout=self.timeout, proxies=self.proxies)
  ...
  def debug(self, verbose=True):
    with StringIO() as output:
      ...
      print("===\n", file=output)
      print("\n", file=output)
      print(f"#res: {len(self)}\n", file=output)
      print(f"code: {self.status_code}\n", file=output)
      ...
      if self.ok:
        for index, result in enumerate(self):
          print("\n", file=output)
          print(f"Details for result #{index + 1}\n", file=output)
          print("---\n", file=output)
